...
// Use src/v3/StableSwapPoolV3.sol for all new deployments (testnet + mainnet).
// V2 used the NON-canonical amplification constant Ann = A*2 instead of the
// canonical Curve Ann = A * n**n = A*4 for a 2-coin pool (see H-1 in the health
// report and test/StableMathComparison.t.sol). V3 restores Ann = A*4.
// Existing V2 testnet deployments (0x938d...) remain functional but are frozen.
// ─────────────────────────────────────────────────────────────────────────────
...
/// @notice Arcis USDC / EURC StableSwap Pool V2 (Curve Amplified Invariant Market Maker).
/// @dev Enhancements in V2:
///      1. MINIMUM_LIQUIDITY (1000) permanently burned to dead address on first deposit
///         to prevent first-depositor share inflation attacks.
///      2. Slippage protection parameter (minLpShares) in addLiquidity.
///      3. Pausable emergency circuit breaker managed by Ownable admin.
///      4. ReentrancyGuard and zero-native-deposit protection.
contract StableSwapPoolV2 is ERC20, ReentrancyGuard, Pausable, Ownable {
    ...
    constructor(
        ...
        ERC20("Arcis USDC-EURC LP v2", "af-USDC-EURC-v2")
