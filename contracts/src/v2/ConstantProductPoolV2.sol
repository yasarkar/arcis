...
// Use src/v3/ConstantProductPoolV3.sol for all new deployments (testnet + mainnet).
// Existing V2 testnet deployments (0x95fd...) remain functional but are frozen.
// ─────────────────────────────────────────────────────────────────────────────
...
/// @notice Arcis USDC / cirBTC Constant-Product Pool V2 (x * y = k).
/// @dev Enhancements in V2:
///      1. Standardized 18-decimal LP token math (scales 7-decimal sqrt to 18 decimals via 1e11).
///      2. MINIMUM_LIQUIDITY (1000) locked to dead address on initial deposit.
///      3. Slippage protection parameters (minLpShares, minOutA, minOutB).
///      4. Pausable emergency breaker managed by Ownable.
contract ConstantProductPoolV2 is ERC20, ReentrancyGuard, Pausable, Ownable {
    ...
    constructor(
        ...
        ERC20("Arcis USDC-cirBTC LP v2", "af-USDC-BTC-v2")
